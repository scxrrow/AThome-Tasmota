...
void WifiConfig(uint8_t type)
{
  if (!Wifi.config_type) {
    ...
    if (WIFI_MANAGER == Wifi.config_type) {
      Wifi.config_type = EX_WIFI_SMARTCONFIG;
    } 
#endif  // USE_WEBSERVER
#ifndef USE_SMARTCONFIG
    if (EX_WIFI_SMARTCONFIG == Wifi.config_type) { Wifi.config_type = WIFI_SERIAL; }
    ...
    else if (EX_WIFI_SMARTCONFIG == Wifi.config_type) {
      AddLog(LOG_LEVEL_INFO, PSTR(D_LOG_WIFI D_WCFG_1_SMARTCONFIG " " D_ACTIVE_FOR_3_MINUTES));
...
{
  AddLog(LOG_LEVEL_DEBUG_MORE, PSTR("param %d"), param);
  AddLog(LOG_LEVEL_DEBUG_MORE, PSTR("counter %d"), Wifi.counter);
  Wifi.counter--;
  switch (param) {
  case WIFI_SERIAL:
  case EX_WIFI_SMARTCONFIG:
  case WIFI_MANAGER:
    WifiConfig(param);
    break;
  default:
    if (Wifi.config_counter) {
      ...
      if (Wifi.config_counter) {
#ifdef USE_SMARTCONFIG
        AddLog(LOG_LEVEL_INFO, PSTR("Wifi.config_type %i"), Wifi.config_type);
        AddLog(LOG_LEVEL_INFO, PSTR("smartConfigDone %i"), WiFi.smartConfigDone());

        if ((EX_WIFI_SMARTCONFIG == Wifi.config_type) && WiFi.smartConfigDone()) {
        ...
        if (!Wifi.config_counter) {
          ...
          AddLog(LOG_LEVEL_INFO, PSTR(D_LOG_WIFI D_WCFG_1_SMARTCONFIG D_CMND_SSID "1 %s"), SettingsText(SET_STASSID1));
        }
      }
      if (!Wifi.config_counter) {
#ifdef USE_SMARTCONFIG
        if (EX_WIFI_SMARTCONFIG == Wifi.config_type) { WiFi.stopSmartConfig(); }
#endif  // USE_SMARTCONFIG
