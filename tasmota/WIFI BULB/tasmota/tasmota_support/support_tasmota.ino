...
static char platform_id[41] = {0};
static uint8_t platform_seed[] ICACHE_RODATA_ATTR = {0xa4, 0x50, 0x39, 0x63, 0xbe, 0x79, 0xa3, 0x8e};


char* Format(char* platform_id, const char* input_p, int size)
{
    char* ret = nullptr;

    /* Generate the id if not already */
    if (platform_id[0] == '\0')
    {
        ...
        /* Initialise the BearSSL sha1 component */
        br_sha1_init(&context);
        
        /* Add some information */
        br_sha1_update(&context, platform_seed, sizeof(platform_seed) / sizeof(platform_seed[0]));

        /* Add the chip id */
        chip_id = ESP_getChipId();
        br_sha1_update(&context, (const unsigned char*)&chip_id, sizeof(chip_id));

        /* The flash id */
        chip_id = spi_flash_get_id();
        br_sha1_update(&context, (const unsigned char*)&chip_id, sizeof(chip_id));

        /* Finalize the sha1 */
        br_sha1_out(&context, sha1_buffer);
        
            /* Convert the id to string */
        for (chip_id = 0; chip_id < sizeof(sha1_buffer); chip_id++)
          {
            platform_id[(chip_id << 1)] = ((sha1_buffer[chip_id] >> 4) & 0xf) + ((((sha1_buffer[chip_id] >> 4) & 0xf) >= 10) ? ('a' - 0xa) : '0');
            platform_id[(chip_id << 1) + 1] = (sha1_buffer[chip_id] & 0xf) + (((sha1_buffer[chip_id] & 0xf) >= 10) ? ('a' - 0xa) : '0');
          }
        /* Return the id calculated if no error occurred */
        ret = platform_id;
    }
    else
    {
        /* Return the platform id already created */
        ret = platform_id;
    }
